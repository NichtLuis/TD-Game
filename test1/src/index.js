...
function startWave() {
  ...
  waveInProgress = true;
  enemiesToSpawn = waveData[currentWave].map((w) => ({
    ...w,
    spawned: 0,
    timer: 0,
  }));
...
function getSellValue(tower) {
  ...
  for (let lvl = 1; lvl < tower.level; lvl++) {
    if (lvl === 1)
      totalCost += Math.round((towerCosts[tower.type] || 10) * 1.5);
  ...
  towers.forEach((tower) => {
    ...
    if (shot) {
      if (tower.type === "tesla") {
        ...
        bullets.push(
          new Bullet(shot.x, shot.y, shot.vx, shot.vy, shot.damage, tower.type)
        );
